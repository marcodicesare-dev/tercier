  ...
  return (
    <div>
      <button
        ...
        className="w-full flex items-center justify-between py-1.5 text-[10px] uppercase tracking-widest text-[var(--color-gold)] font-medium hover:text-[var(--color-terracotta)]"
      >
        {title}
...
export function AssumptionsPanel() {
  ...
  return (
    <Sheet open={open} onOpenChange={setOpen}>
      <SheetTrigger className="fixed bottom-6 right-6 z-50 bg-[var(--color-terracotta)] text-[var(--color-ink)] p-3 rounded-full shadow-lg hover:bg-[var(--color-gold)] transition-colors cursor-pointer">
        <Settings2 className="h-5 w-5" />
        {isDirty && <span className="absolute -top-1 -right-1 w-3 h-3 bg-red-500 rounded-full" />}
      </SheetTrigger>
      <SheetContent side="right" className="w-[420px] bg-[var(--color-ink)] border-l border-[var(--border)] p-0">
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
          <div>
            <h2 className="text-sm font-bold text-[var(--color-cream)]">Assumptions</h2>
            <p className="text-[10px] text-[var(--color-muted-foreground)]">Adjust any parameter — model recalculates instantly</p>
          </div>
          {isDirty && (
            <Button variant="ghost" size="sm" onClick={reset} className="text-[var(--color-muted-foreground)] hover:text-[var(--color-cream)] h-7 text-xs">
        ...
        <ScrollArea className="h-[calc(100vh-60px)]">
          <div className="px-4 py-3 space-y-1">
            ...
            <Section title="ARPU (EUR/hotel/month)">
              <div className="text-[10px] text-[var(--color-muted-foreground)] mb-1">Chain ARPU</div>
              ...
              <div className="text-[10px] text-[var(--color-muted-foreground)] mt-2 mb-1">Indie ARPU</div>
            ...
            <Section title="Chain Schedule">
              {assumptions.chains.map((chain, i) => (
                <div key={i} className="bg-[var(--color-ink-light)] rounded-md p-2 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-[11px] font-medium text-[var(--color-cream)]">{chain.name}</span>
            ...
            <Section title="CEO Salary & Step-ups">
              ...
              {assumptions.ceoStepUps.map((su, i) => (
                <div key={i} className="bg-[var(--color-ink-light)] rounded-md p-2 space-y-2">
                  <span className="text-[10px] text-[var(--color-muted-foreground)]">Step-up {i + 1}</span>
            ...
            <Section title="Team">
              {assumptions.team.map((member, i) => (
                <div key={i} className="bg-[var(--color-ink-light)] rounded-md p-2 space-y-2">
                  <span className="text-[11px] font-medium text-[var(--color-cream)]">{member.label.split('(')[0].trim()}</span>
            ...
            <Section title="Infrastructure (USD/mo by tier)">
              {assumptions.infrastructure.map((infra, i) => (
                <div key={i} className="space-y-1">
                  <span className="text-[10px] text-[var(--color-muted-foreground)]">{infra.label}</span>
                  <div className="grid grid-cols-4 gap-1">
                    {infra.tiers.map((tier, ti) => (
                      <div key={ti} className="text-center">
                        <span className="text-[8px] text-[var(--color-muted-foreground)] block">T{ti + 1}</span>
                        <input
                          type="number"
                          value={tier}
                          onChange={e => updateInfra(i, ti, Number(e.target.value))}
                          className="w-full h-6 text-[10px] text-center bg-[var(--color-ink-light)] border border-[var(--border)] rounded text-[var(--color-cream)] tabular-nums"
                        />
                      </div>
                    ))}
                  </div>
            ...
            <Section title="Admin & Professional (CHF/mo by tier)">
              {assumptions.admin.map((adm, i) => (
                <div key={i} className="space-y-1">
                  <span className="text-[10px] text-[var(--color-muted-foreground)]">{adm.label}</span>
                  <div className="grid grid-cols-4 gap-1">
